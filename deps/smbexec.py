...
class SMBServer(Thread):
    ...
    def run(self):
        ...
        self.smb = smbserver.SMBSERVER(
            ('0.0.0.0', 445), config_parser=smbConfig)
    ...
    def run(self, remoteName, remoteHost):
        ...
        try:
            ...
            self.shell = RemoteShell(
                self.__share, rpctransport, self.__mode, self.__serviceName, self.__command)
...
class RemoteShell(cmd.Cmd):
    def __init__(self, share, rpc, mode, serviceName, command):
        ...
        if mode == 'SERVER':
            ...
            self.__copyBack = 'copy %s \\\\%s\\%s' % (
                self.__output, myIPaddr, DUMMY_SHARE)
    ...
    def finish(self):
        ...
        try:
            ...
            resp = scmr.hROpenServiceW(
                self.__scmr, self.__scHandle, self.__serviceName)
    ...
    def get_output(self):
        ...
        if self.__mode == 'SHARE':
            self.transferClient.getFile(
                self.__share, OUTPUT_FILENAME, output_callback)
