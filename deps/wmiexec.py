...
"""
    ...
    def default(self, line):
        ...
        if len(line) == 2 and line[1] == ':':
            ...
            if len(self.__outputBuffer.strip('\r\n')) > 0:
                ...
                self.execute_remote('cd ')
                self.__pwd = self.__outputBuffer.strip('\r\n')
                self.prompt = unicode(self.__pwd + '>').encode(
                    sys.stdout.encoding)
                self.__outputBuffer = ''
        else:
            if line != '':
    ...
    def get_output(self):
        ...
        while True:
            try:
                self.__transferClient.getFile(self.__share, self.__output,
                                              output_callback)
                break
            except Exception, e:
...
def load_smbclient_auth_file(path):
    ...
    for line in open(path):
        ...
        if k == 'username':
            ...
            raise AuthFileSyntaxError(path, lineno,
                                      'Unknown option %s' % repr(k))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nooutput',
                        action='store_true',
                        default=False,
                        help='whether or not to print the output '
    ...
    try:
        ...
        executer = WMIEXEC(' '.join(options.command), username, password,
                           domain, options.hashes, options.aesKey,
