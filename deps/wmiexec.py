...
"""
    ...
    def do_cd(self, s):
        ...
        if len(self.__outputBuffer.strip('\r\n')) > 0:
            ...
            self.__pwd = ntpath.normpath(ntpath.join(
                self.__pwd, s.decode(sys.stdin.encoding)))
    ...
    def default(self, line):
        ...
        if len(line) == 2 and line[1] == ':':
            ...
            if len(self.__outputBuffer.strip('\r\n')) > 0:
                ...
                self.__pwd = self.__outputBuffer.strip('\r\n')
                self.prompt = unicode(
                    self.__pwd + '>').encode(sys.stdout.encoding)
                self.__outputBuffer = ''
        else:
            if line != '':
                self.send_data(line)
    ...
    def execute_remote(self, data):
        ...
        self.__win32Process.Create(command.decode(
            sys.stdin.encoding), self.__pwd, None)
        # hereyyy
        self.get_output()
    ...
    for line in open(path):
        ...
        if k == 'username':
            ...
            raise AuthFileSyntaxError(
                path, lineno, 'Unknown option %s' % repr(k))
